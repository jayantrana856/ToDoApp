...
def test_create__todo(test_todo):
    request_data = {
        'title': 'New Todo!',
...
def test_delete_todo(test_todo):
    request_data = client.delete('/todos/todo/1')
    assert request_data.status_code == 204
    db = TestingSessionLocal()
    model = db.query(ToDos).filter(ToDos.id == 1).first()
    assert model is None

def test_delete_todo_not_found(test_todo):
    request_data = client.delete('/todos/todo/999')
    assert request_data.status_code == 404
    assert request_data.json() == {'detail': 'todo not found'}
